...
/**
 * CnNode.java - A Classifier (novel) Node class. Edited version of CNode.java
 * @author	Maxwell Sime
 * @version January 2021
 */
public class CnNode implements Serializable {
	...
	/**
	 * CnNode - A Node 'j', taking inputs from all parents inX and paY.
	 ...
	 */
	public CnNode(int j, int inX[], int paY[]) {
		this.j = j;
		this.inX = inX;
		this.paY = paY; 
	}
	...
	public Instances transform(Instances D) throws Exception {
		int L = D.classIndex();
		d = D.numAttributes() - L;
		int keep[] = {j};				// keep current node label
		Arrays.sort(keep);
	...
	*/
	...
	public static void main(String args[]) throws Exception {
		...
		for(int j : s) {
			...
			CnNode n = new CnNode(j,null,pa_j);
			n.build(D,new SMO());
			/*
			 */
